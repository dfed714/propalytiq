...
const PropertyPreview: React.FC<PropertyPreviewProps> = ({ 
  ...
  const [showOptionalInputs, setShowOptionalInputs] = React.useState(false);
    ...
    <div className="space-y-8">
      <Card className="overflow-hidden border-purple-200">
        ...
        <CardContent className="p-6">
          <h3 className="text-2xl font-semibold mb-2">{propertyData.address}</h3>
      ...
      <div className="bg-white rounded-xl border border-purple-200 p-4 sm:p-6 space-y-6">
        <div>
          <h3 className="text-xl font-semibold mb-4">Select Your Investment Strategy</h3>
          <Select onValueChange={setInvestmentStrategy}>
            ...
            <SelectContent>
              ...
              <SelectItem value="sa">Serviced Accommodation / Airbnb</SelectItem>
              <SelectItem value="hmo">HMO (House in Multiple Occupation)</SelectItem>
            </SelectContent>
          </Select>
          
          {investmentStrategy && (
            <div className="mt-4 p-4 purple-gradient-subtle rounded-lg border border-purple-200">
              <h4 className="font-medium text-purple-700">
                {investmentStrategy === 'btl' && 'Buy-to-Let Strategy'}
                {investmentStrategy === 'brr' && 'Buy-Refurbish-Refinance Strategy'}
                {investmentStrategy === 'sa' && 'Serviced Accommodation Strategy'}
                {investmentStrategy === 'hmo' && 'HMO Strategy'}
              </h4>
              <p className="mt-2 text-sm text-gray-600">
                {investmentStrategy === 'btl' && 'Purchase a property with the primary aim of renting it out to tenants for regular monthly income.'}
                {investmentStrategy === 'brr' && 'Buy a property below market value, renovate to increase its value, then refinance to pull out capital for your next investment.'}
                {investmentStrategy === 'sa' && 'Offer short-term, fully furnished accommodation to guests, typically at higher rates than standard rentals.'}
                {investmentStrategy === 'hmo' && 'Rent individual rooms to multiple tenants who share common facilities like kitchens and bathrooms.'}
              </p>
            </div>
          )}
        </div>
        ...
        <Accordion
          type="single" 
          collapsible
          onValueChange={(value) => setShowOptionalInputs(!!value)}
        >
          <AccordionItem value="optional-inputs">
            <AccordionTrigger>Additional Investment Details (Optional)</AccordionTrigger>
            <AccordionContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                ...
                <div>
                  <Label htmlFor="monthly-rental">Expected Monthly Rental (£)</Label>
                  <Input
                    id="monthly-rental"
        ...
        <div className="pt-4">
          <Button 
            ...
            className="w-full purple-gradient-bg hover:opacity-90 transition-opacity"
            size="lg"
          >
